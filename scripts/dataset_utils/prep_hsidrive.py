...
def convert_dir(dataset_name, inputdir, outdir, split, split_file):
    sample_names = read_split_file(split_file)
    datadir = inputdir.joinpath('cubes_fl32')
    labeldir = inputdir.joinpath('labels')

    outfile = outdir.joinpath(f'{dataset_name}_{split}.h5')

    print(f"{len(sample_names)} samples in {split} split")
    ...
    with h5py.File(outfile, "a") as hdf_file:
        for sample in tqdm(sample_names):
            if sample in hdf_file:
                continue
            imgpath = datadir.joinpath(f'{sample}_TC.mat')
            labelpath = labeldir.joinpath(f'{sample}.png')
            mat = loadmat(imgpath)
            data = np.transpose(mat.get('cube'), (1,2,0))
            labels = imread(labelpath)
            
            if (data is not None and labels is not None):
                group = hdf_file.create_group(sample)
                group.create_dataset("image", data=data)
                group.create_dataset("labels", data=labels)
            else:
                print(f"{sample} is missing or empty.")
    t2 = time.time()
    print(f"Converted {len(sample_names)} in {t2-t1}s!")
    print()
