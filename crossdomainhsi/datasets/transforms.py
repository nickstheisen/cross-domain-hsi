...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        label = torch.from_numpy(label).unsqueeze(0).type(torch.long)
...
class RemoveChannels(object):
    """ Removes all channels defined in `rem_ch`. Expects numpy array (H, W, C)"""
    def __init__(self, rem_ch):
        self.rem_ch = rem_ch

    def __call__(self, sample):
        patch, label = sample
        patch = np.delete(patch, self.rem_ch, axis=-1)
        return patch, label
...
class Normalize(object):
    ...
    def __call__(self, sample):
        patch, label = sample
        patch = (patch - self.min) / (self.max - self.min)
